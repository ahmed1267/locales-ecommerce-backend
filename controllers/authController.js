...
exports.login = asyncHandler(async (req, res, next) => {
  ...
  if (!user || !isCorrectPassword) {
    return next(new ApiError(req.t('incorrect email or password')), 401);
  }
  // 3) Generate token
  const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET, {
    expiresIn: process.env.JWT_EXPIRES_IN,
...
exports.auth = asyncHandler(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new ApiError(req.t('You are not logged in. Please login to get access'), 401)
    );
  }
  ...
  if (!currentUser) {
    return next(
      new ApiError(req.t('The user that belong to this token does no longer exist'))
    );
  }
  ...
  if (currentUser.passwordChangedAt) {
    ...
    if (passChangedTimestamp > decoded.iat) {
      return next(
        new ApiError(
          req.t("User recently changed password! Please login again.."),
          401
        )
      );
    }
...
exports.allowedTo = (...roles) =>
  asyncHandler(async (req, res, next) => {
    ...
    if (!roles.includes(req.user.role)) {
      return next(
        new ApiError(req.t("You are not allowed to perform this action"), 403)
      );
    }
    next();
  });
...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ApiError(req.t('There is no user with this email address'), 404)
    );
  }
  ...
  const message = req.t("Forgot your password? Submit this reset password code :")`+${resetCode}\n` + req.t("If you didn't forgot your password, please ignore this email!");

  try {
    await sendEmail({
      email: user.email,
      subject: req.t("Your Password Reset Code (valid for 10 min)"),
      message,
    });

    res.status(200).json({
      status: req.t("success"),
      message: req.t("Reset code sent to your email"),
      // userId: user.id,
    });
  } catch (err) {
...
exports.verifyPasswordResetCode = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    return next(new ApiError(req.t("Reset code is invalid or has expired"), 400));
  }
...
exports.resetPassword = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ApiError(
        req.t("There is no user with this email address") + `${req.body.email}`,
        404
      )
    );
  }
  ...
  if (!user.resetCodeVerified) {
    return next(new ApiError(req.t("reset code not verified"), 400));
  }
