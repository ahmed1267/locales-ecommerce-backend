...
exports.addProductToCart = asyncHandler(async (req, res, next) => {
  ...
  return res.status(200).json({
    ...
    message: req.t('Product added successfully to your cart'),
...
exports.updateCartProductCount = asyncHandler(async (req, res, next) => {
  ...
  if (itemIndex > -1) {
    ...
    return next(
      new ApiError(req.t('No Product Cart item found for this id:') + `${itemId}`)
    );
  }
  // Calculate total cart price
  await calcTotalCartPrice(cart);
...
exports.applyCouponToCart = asyncHandler(async (req, res, next) => {
  ...
  if (!coupon) {
    ...
    cart.coupon = undefined;
    await cart.save();
    return next(new ApiError(req.t('Coupon is invalid or has expired'), 400));
  }
